...
class EventStudyAnalyzer:
    ...
    def identify_high_impact_events(
        ...
        try:
            ...
            results = self.db.execute_query(query, [start_date, end_date, min_sentiment_threshold])
            ...
            df = pd.DataFrame(results, columns=[
                'news_id', 'timestamp', 'title', 'sentiment_score', 
                'sentiment_label', 'confidence', 'economic_indicators', 
                'fed_officials', 'is_high_impact'
            ])
    ...
    def measure_market_impact(
        ...
        try:
            ...
            df['time_diff'] = abs((df['timestamp'] - event_timestamp).dt.total_seconds())
            ...
            return {
                ...
                'volatility_ratio': post_volatility / pre_volatility if pre_volatility != 0 else 1.0,
            }
    ...
    def conduct_event_study(
        ...
        # Identify events
        events_df = self.identify_high_impact_events(start_date, end_date, min_sentiment)
        ...
        for label in ['risk-on', 'risk-off', 'neutral']:
            ...
            results[label] = {
                ...
                'consistent_direction': (
                    ...
                    if label == 'risk-on' else
                    (subset['absolute_change'] < 0).sum() / len(subset) * 100
                    if label == 'risk-off' else 50.0
                ),
            }
    ...
    def test_significance(
        ...
        try:
            ...
            groups = [
                event_study_df[event_study_df['sentiment_label'] == label]['percent_change'].values
                for label in ['risk-on', 'risk-off', 'neutral']
            ...
            return {
                ...
                'sentiment_labels_differ': anova_p < 0.05 if anova_p is not None else None,
            }
    ...
    def run_full_event_study(
        ...
        return {
            ...
            'events': event_study_df.to_dict('records') if len(event_study_df) <= 100 else []
        }
